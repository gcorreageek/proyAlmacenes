...
public class MySqlProductoDAO implements ProductoDAO {
		...
		try {
			if(producto==null){ 
				...
				if(!producto.getDesc_producto().isEmpty()){
					lstProductos = (List<ProductoDTO>) sesion.selectList("producto.SQL_listaProductosDescProducto","%"+producto.getDesc_producto()+"%");
				} 
			}
		} catch (Exception e) {
			System.out.println(e);
	...
	public List<ProductoDTO> buscarProductosPaginados(ProductoDTO producto,
		...
		try {
			if(producto==null){  
				...
				lstProductos = (List<ProductoDTO>) sesion.selectList("producto.SQL_listaProductosPaginados",prod);
			}else{ 
				if(!producto.getDesc_producto().isEmpty()){
					producto.setDesc_producto("%"+producto.getDesc_producto()+"%");
					producto.setInicio(inicio);
					producto.setTamano(tamano);
					lstProductos = (List<ProductoDTO>) sesion.selectList("producto.SQL_listaProductosDescProductoPaginados",producto);
				} 
			}
		} catch (Exception e) {
			System.out.println(e);
		...
		try {
			if(productoViene!=null){  
				if(productoViene.getCod_producto()!=null){
					producto = (ProductoDTO) sesion.selectOne("producto.SQL_getProductoXCodProd",productoViene.getCod_producto());
				} 
			}
		} catch (Exception e) {
			System.out.println(e);
	...
	public Boolean eliminarProducto(ProductoDTO objProducto) {
		...
		SqlSession session = sqlMapper.openSession();
		try {
				session.delete("producto.SQL_deleteProducto",objProducto.getCod_producto());
				session.commit();
				result= true;
		} catch (Exception e) {
			System.out.println(e);
